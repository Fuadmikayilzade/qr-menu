...
const WHATSAPP_NUMBER = "994104195344"; // 0104195344 -> +994104195344
...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Mesaj formatı (WhatsApp üçün)
    const whatsappText = `
📩 Yeni əlaqə mesajı (DigiMenu.az)

👤 Ad: ${formData.name}
📞 Əlaqə: ${formData.contact}
...
${formData.message}
    ...
    // Prompt mesaj
    setResponseMessage("WhatsApp açıldı. Mesajı göndərin ✅");
    setShowPrompt(true);
  ...
  // Prompt mesajını 2 saniyəyə bağla
  useEffect(() => {
    if (showPrompt) {
      const timer = setTimeout(() => {
        setShowPrompt(false);
  ...
  return (
    <div className="contact">
      <div className="contact-header">
        ...
        <h2>Suallarınızı bizə göndərin</h2>
        <p>
          Hər hansı sualınız və ya təklifiniz varsa, bizimlə əlaqə saxlamaqdan
          çəkinməyin.
        </p>
      </div>

      {showPrompt && (
        <div className="prompt-message success">{responseMessage}</div>
      )}
      ...
      <div className="contact-footer">
        <h2>Bizimlə Əlaqə</h2>
        <p>
          Əlaqə nömrəsi: <a href="tel:+994104195344">+994 10 419 53 44</a>
        </p>
        <p>
          Email: <a href="mailto:info@digimenu.az">info@digimenu.az</a>
        </p>
